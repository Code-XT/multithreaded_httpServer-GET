...
#define MAX_BYTES 4096      			//max buffer size
#define MAX_CLIENTS 400     			//max number of clients
#define MAX_SIZE 200*(1<<20)           	//max size of cache
#define MAX_ELEMENT_SIZE 10*(1<<20)     //max size of an element in the cache
typedef struct cache_element{
    char* data;         		//data stores the data of the element
    int len;            		//len stores the length of the response
    char* url;          		//url stores the url of the request
	time_t lru_time_track;    	//lru_time_track stores the time of last access
    cache_element* next;       	//next stores the pointer to the next element  
} cache_element;
...
int proxy_socketId;					//proxy socket id
pthread_t tid[MAX_CLIENTS];         //array of thread ids
sem_t seamaphore;	                //seamaphore is used for limiting the number of clients


pthread_mutex_t lock;               //lock for cache


cache_element* head;                //head stores the head of the cache
int cache_size;                     //cache_size stores the size of the cache
...
if (ParsedHeader_get(request, "Host") == NULL) {
    if (ParsedHeader_set(request, "Host", request->host) < 0) {
        printf("Failed to set the 'Host' header.\n");
    }
}
...
if (ParsedRequest_unparse_headers(request, buf + len, (size_t)MAX_BYTES - len) < 0) {
	...
	if (remoteSocketID < 0) {
    printf("Failed to create socket for the remote server.\n");
    return -1;
	}
	...
	while(bytes_send > 0)
	{
		...
		if (bytes_send < 0) {
		perror("Failed to send data to the client socket.\n");
		break;
		}
...
int main(int argc, char * argv[]) {
	...
	if (listen_status < 0) {
    	perror("Failed to listen on the proxy socket.\n");
    	exit(1);
	}
	printf("Proxy server is now listening for incoming connections.\n");
...
// Checks for url in the cache if found returns pointer to the respective cache element or else returns NULL
	...
	printf("Cache lock acquired for removal: %d\n",temp_lock_val); 
    if(head!=NULL){
        site = head;
        while (site!=NULL)
        {
            if(!strcmp(site->url,url)){
                ...
                printf("\nCache hit: URL found. Updating LRU time track.\n");
    ...
    printf("\nCache miss: URL not found in the cache.\n");
	}
    temp_lock_val = pthread_mutex_unlock(&lock);
	printf("\nCache lock released after removal: %d\n",temp_lock_val); 
    return site;
}
...
void remove_cache_element(){
    ...
    temp_lock_val = pthread_mutex_unlock(&lock);
	printf("Cache lock released after removal: %d\n",temp_lock_val); 
}

int add_cache_element(char* data,int size,char* url){
    // Adds element to the cache
    int temp_lock_val = pthread_mutex_lock(&lock);
	printf("Cache lock acquired for addition: %d\n", temp_lock_val);
    ...
    if(element_size>MAX_ELEMENT_SIZE){
        ...
        temp_lock_val = pthread_mutex_unlock(&lock);
		printf("Cache lock released after addition: %d\n", temp_lock_val);
        return 0;
    }
    else
    {   while(cache_size+element_size>MAX_SIZE){
        ...
        element -> url = (char*)malloc(1+( strlen( url )*sizeof(char)  )); // Allocating memory for the url to be stored in the cache element
